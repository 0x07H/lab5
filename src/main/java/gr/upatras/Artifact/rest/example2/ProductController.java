...
@ApiOperation(value = "Updates partially a Product", nickname = "patchProduct", notes = "This operation updatespartially a	Product entity.", response = Product.class )
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Updated", response = Product.class),
			...
			@ApiResponse(code = 409, message = "Conflict", response = Error.class),
			@ApiResponse(code = 500, message = "Internal Server Error", response = Error.class) })
	@RequestMapping(value = "/product/{id}", produces = { "application/json;charset=utf-8" }, consumes = {
